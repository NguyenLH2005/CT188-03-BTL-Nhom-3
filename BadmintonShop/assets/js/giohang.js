// Code trang giỏ hàng
var iconCart = document.querySelector('.page-header__tool-item .fa-cart-shopping');
...
{
    var listButtonsCart = document.querySelectorAll('.content .descriptionItem i'); //Lấy ra tất cả các nút 
    listButtonsCart.forEach(function(element, index)
    {
        element.addEventListener('click',function(event)
        {
            ...
            let newItem =
            {
                ...
                priceItem: parseInt(item.querySelector('.descriptionItem .infoOtherItem .costItem').textContent.replace(/\D/g,'')),
                quantity: 1
            }
            if(JSON.parse(localStorage.getItem('cartItems')) === null)  // Chưa có giỏ hàng
            {
                alert('Sản phẩm '+item.querySelector('.descriptionItem .nameItem').textContent+' đã được thêm vào giỏ hàng thành công!');
        ...
        customerCart.forEach(function(item, index)
        {
            bodyContent +=    
                    '<tr class="page-main__shoppingCart__item">'+
                        '<td>'+ (index + 1) +'</td>'+
                        '<td>'+
                            '<img src="'+item.imageItem+'" alt="">'+
                        '</td>'+
                        '<td>'+item.nameItem+'</td>'+
                        '<td>'+'<input type="number" min="1" value="'+item.quantity+'">'+'</td>'+
                        '<td>'+item.priceItem+'<sup>đ</sup></td>'+
...
function deleteCart() {
    let trash = document.querySelectorAll('.page-main__shoppingCart__item .fa-trash');
    let customerCart = JSON.parse(localStorage.getItem('cartItems'));
